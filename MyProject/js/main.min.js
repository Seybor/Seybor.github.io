...
if (document.querySelector('#template-words') !== null) {
  ...
  const randomNumber = (min = 0, max = WORDS__ARRAY.length - 1) => {
    return Math.floor(Math.random() * (max - min + 1) + min);
  };
  
  const checkEqualNumber = () => {
    let check;
    let Arr = [];
    do {
      for (let i = 0; i < 5; i++) {
      ...
      console.log(Arr);
      error: for (let i = 0; i < Arr.length - 1; i++) {
        check = Arr[i];
  ...
  const Arr = checkEqualNumber();
  
  for (let i = 0; i < Arr.length; i++) {
    console.log(WORDS__ARRAY[Arr[i]].ru);
    console.log(
      WORDS__ARRAY[randomNumber()].en + ' , ' + WORDS__ARRAY[randomNumber()].en + ' , ' + WORDS__ARRAY[Arr[i]].en
    );
  }
  ...
  const renderWords = function (words) {
    var wordsElement = template.cloneNode(true);
    ...
    for (let i = 0; i < 4; i++) {
      wordsInputs[i].name = WORDS__ARRAY[0].en;
      wordsSpan[i].textContent = WORDS__ARRAY[i].en;
    }
  ...
  const renderStart = (value, lang) => {
    ...
    if (lang === 'en') {
      for (var i = 0; i < value; i++) {
        fragment.appendChild(renderWords(WORDS__ARRAY[i].en));
      }
    }
...
const headerLinksCurrent = [];
headerLinks.forEach((el) => {
  headerLinksCurrent.push(el.textContent);
});

{
  ...
  if (screenWidth <= 600) {
    ...
    headerLinks.forEach((el, id) => {
      switch (id) {
        case 0:
          el.textContent = 'Offers';
          break;
        case 1:
    ...
    headerLinks.forEach((el, id) => {
      el.textContent = headerLinksCurrent[id];
    });
  }
}
