...
const generatesPhotos = (maxPhoto = 25) => {
  ...
  for (let i = 0; i < maxPhoto; i++) {
    ...
    const randomCommentsNumber = (min = 1, max = 10) => {
      return Math.floor(Math.random() * (max - min + 1) + min);
    };
    const obj = {};
    obj.url = i + 1;
...
var renderComments = function (value) {
  var socialCommentsElement = socialCommentsTemplate.cloneNode(true);
  const randomNumber = (min = 2, max = 6) => {
    return Math.floor(Math.random() * (max - min + 1) + min);
  };

  socialCommentsElement.querySelector('.social__picture').src = `img/avatar-${randomNumber()}.svg`;
...
for (let i = 0; i < mainArray[0].comments.length; i++) {
  Commentsfragment.appendChild(renderComments(mainArray[0].comments[i]));
}
